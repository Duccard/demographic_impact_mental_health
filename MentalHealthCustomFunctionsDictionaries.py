...
def plot_age_distribution_all_years(merged_df: pd.DataFrame) -> pd.Series:
    ...
    # Filter only rows with Age question responses (no year restriction)
    age_df = merged_df[merged_df["questiontext"] == "Age"][
        ["UserID", "AnswerText"]
    ].copy()

    # Convert age to numeric, ignore non-numeric entries
    age_df["Age"] = pd.to_numeric(age_df["AnswerText"], errors="coerce")

    # Drop duplicates to ensure unique respondents
    ...
    # Count respondents by age group
...
def plot_question_116_conditions_horizontal(
    ...
    try:
        ...
        condition_series = (
            unique_df["AnswerText"]
            .str.split(",|;|/|\\|", expand=True)
            .stack()
            .str.strip()
        )
        condition_series = condition_series[condition_series != ""].dropna().str.lower()

        categorized_conditions = condition_series.apply(
            lambda x: categorize_condition(x, condition_mapping)
        )

        other_conditions = condition_series[categorized_conditions == "Other"]
        other_counts = other_conditions.value_counts().sort_values(ascending=False)
        ...
        if not other_percentages.empty:
            print("\nBreakdown of 'Other' Conditions (% within 'Other'):\n")
            for condition, percentage in other_percentages.items():
                print(f"- {condition.capitalize()}: {percentage:.1f}%")
...
def plot_question_33_bar(merged_df: pd.DataFrame) -> None:
    try:
        # Filter for Question ID 33 (all years)
        filtered_df: pd.DataFrame = merged_df[merged_df["QuestionID"] == 33].copy()

        # Keep only unique respondents
        ...
        for index, row in plot_df.iterrows():
            plt.text(
                row["Percentage"] + 0.5,  # Adjust text position if needed
                index,
                f"{row['Percentage']:.1f}%",
                va="center",
            )
...
def plot_multibar_question33_by_gender_percentage_all_years(
    ...
    try:
        ...
        # Optional: print out the percentages
...
def plot_global_country_most_common_answer_q33(
    ...
    try:
        ...
        ax.set_title(
            "Most Common Answer to Question 33 by Country (All Years)", fontsize=14
        )
        ax.axis("off")
